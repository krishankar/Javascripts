...
var pass = passwordStrength("manikyam");
console.log(pass);
...
var firstInDictionary = function(word1, word2, word3)
{
    ...
    {
        return "if any of the arguments are not strings";
    }
};
dict = firstInDictionary("man", "king", "trush");
console.log(dict);
...
var qs = isQuestion("are you working?");
console.log(qs);
...
var magic8Ball = function(question)
{
    ...
    if (typeof question === 'string')
    {
        ...
        if (q == "?")
        {
            var responce = ['Signs points to yes', 'Cannot predict now',
                'It is certain', 'my reply is no', 'very doubtful',
                'Yes definitely', 'My sources say no', 'As I see it, yes'
            ];
...
var qs = magic8Ball("do like icecreams");
console.log(qs);
...
var interjectAt = function(interjection, slicelength, tweet)
{
    if (typeof interjection === 'string' && typeof slicelength ===
        'number' && typeof tweet === 'string')
    {
        if (tweet.length > slicelength)
        {
            var beginning = tweet.slice(0, slicelength);
            var end = tweet.slice(slicelength, tweet.length);
            var strWithInterjection = beginning + " " + interjection + " " +
                end;
            return strWithInterjection;
        }
        else
        {
            return "he string doesn't have that many letters!";
        }
    }
    else
    {
...
var tweets = interjectAt("this is joke", 5, "Hello CodingSastra");
console.log(tweets);
...
var randomInterject = function(tweet)
{
    ...
    {
        var interjection = ["Good", "nice", "super", "excellet",
            "marvellous"
        ];
        var ilength = interjection.length;
        var interjectrand = Math.floor(Math.random(ilength) * ilength);
        var interject = interjection[interjectrand];
        var random = Math.floor(Math.random(tweetlength) * tweetlength);
        var tweetlength = tweet.length;
        var random = Math.floor(Math.random(tweetlength) * tweetlength);
        var beginning = tweet.slice(0, random);
        var end = tweet.slice(random, tweetlength);
        var strWithInterjection = beginning + " " + interject + " " + end;
        return strWithInterjection;
    }
    else
    {
        return "expected arg to be a string";
    }
};
var tweets = randomInterject("Hello CodingSastra");
console.log(tweets);
